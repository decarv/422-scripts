...
PUBLIC char lockpriority(pid_t __pid, char __priority)
{
    message m;
...
PUBLIC char __lockpriority()
{
  int __prmp_nr, __crmp_nr;
  struct mproc *__prmp;
  struct mproc *__crmp;

  /* verify priority validity */
  if (m_in.__priority < PRIO_MIN || m_in.__priority > PRIO_MAX) {
    return -1;
  }

  /* verify pid validity */
  if ((__prmp_nr = proc_from_pid(m_in.__parent_pid)) < 0) { /* get process number */
    return PRIO_MIN-1; /* shouldnt happen */
  }
  __prmp = &mproc[__prmp_nr]; /* get process info */
  if ((m_in.__child_pid < 0) || (__crmp_nr = proc_from_pid(m_in.__child_pid)) < 0) {
    return PRIO_MIN-1; /* shouldnt happen */
  }
  __crmp = &mproc[__crmp_nr];
  if (crmp->mp_parent != prmp_nr) {
      return -2;
  }

  crmp->mp_nice = __priority;
  return sys_nice(crmp->mp_endpoint, __priority);
}

PUBLIC char unlockpriority(pid_t __pid)
{
    message m;
    _syscall(MM, UNLOCKPRIORITY, &m);
}

PUBLIC void __unlockpriority()
{
    return ; /* TODO */
}
